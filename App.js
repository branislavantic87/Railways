...
class App extends Component {
  ...
  isLoading() {
    ...
    const pathProjectJson = dirs.DocumentDir + '/projectJson.json';
    ...
    compareJsonAndDlNew = async () => {
      ...
      .then(res => {
        if (res !=true) {
          tempDontExist()
          .then(async () => { await RNFB.fs.readFile(pathToTemp, 'utf8')
      ...
      console.log('bbbbbb');
      let t0 = Date.now();
      await RNFB.fs.readFile(pathToContentJson, 'utf8') // ocitaj
        .then(fileAsString => {
          ...
          if (tempJson.hash != contentJsonObj.hash) { // ako su hash-evi isti
    ...
    downloadOne = (file) => {
      return RNFB.config({
        path: pathToFiles + file.fileId + '.' + file.ext,
        fileCache: true
      })
        .fetch('GET', uri, {})
        .then((r) => {
